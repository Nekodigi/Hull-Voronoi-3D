...
class ExampleConstrainedDelaunay {
  ...
  ExampleConstrainedDelaunay(int dim, ArrayList<ArrayList<float[]>> constraintss, boolean shouldRemoveTriangles) {
    //randomSeed(seed);
    ...
    for(ArrayList<float[]> constraints : constraintss){
    ...
    ArrayList<Simplex> triangles = he.toSimplexes();
    polygons = simplex2Poly(triangles);
  }
