class Voronoi{
  ...
  ArrayList<Polygon> polygons = new ArrayList<Polygon>();//voronoi vertices (circum center of simplexes)
  Delaunay delaunay;
  ...
  void Generate(ArrayList<Vertex> input){
    ...
    for(int i = 0; i < delaunay.simplexes.size(); i++){//calculation all circumCenter
      Simplex simplex = delaunay.simplexes.get(i);
      simplex.calcCircumCenter();
      simplex.circumC.id = i;
      Vvertices.add(simplex.circumC);
    }
